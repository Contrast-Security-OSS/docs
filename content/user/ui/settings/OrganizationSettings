...
## Score Settings
Organization administrators can customize score settings for libraries. This setting provides the ability to configure vulnerability-only scoring or to use the default scoring algorithm, which includes the date and versioning of a library. 
...
## Library Policy
To manage library compliance policy, select **Organization Settings** in the user menu and then **Library Policy** in the sidebar. In the **Library Compliance** settings, you have the option to fail any library that violates a set policy. Note that this choice will appear as an alert in **Score Settings**.
