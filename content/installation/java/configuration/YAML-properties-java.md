...
* **protect**:
  ...
  * **rules**:
    ...
    * **path-traversal**:
      * **mode**: Set the mode of the rule. Value options are `monitor`, `block`, `block_at_perimeter`, or `off`. <br> Note: If a setting says, "if blocking is enabled", the setting can be `block` or `block_at_perimeter`.
      * **aggressive_comment**: Tell the agent to be more aggressive in detecting user inputs as SQL comments. This will enable the agent to better detect SQL Injection input vectors that use comments to terminate queries. The agent blocks if blocking is enabled.
