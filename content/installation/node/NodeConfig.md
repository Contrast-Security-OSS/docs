...
--agent.service.enable <enable>                | AGENT\_\_SERVICE\_ENABLE                        | Set to `false` to disallow the service to be started, and effectively disable the agent, if read by the service. If the agent reads this property, it disallows service auto-start.
--agent.service.socket                         | AGENT\_\_SERVICE\_SOCKET                        | If this property is defined, the service is listening on a Unix socket at the defined path. Example: /tmp/service.sock
--agent.service.host                           | AGENT\_\_SERVICE\_HOST                          | Set the the hostname or IP address of the Contrast service to which the Contrast agent should report.
--agent.service.port                           | AGENT\_\_SERVICE\_PORT                          | Set the the port of the Contrast service to which the Contrast agent should report. **Required**. Example: `30555`
